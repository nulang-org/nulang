...
The JIT no longer stores a `&'static [Constant]`-like raw slice in thread-local
state. Interned-string helpers resolve through the active VM pointer plus an
explicit module index, creating only a short-lived shared borrow for the
lookup. `VM::try_jit_execute` also prepares code first, then moves the JIT
backend and raw-bit constant cache out of `VM` before native execution, so a
re-entrant `nulang_jit_direct_call` cannot alias the same mutable JIT backend
through `VM::step()`. Debug assertions at `VM::jit_direct_call` enforce
that ownership invariant in tests/debug builds.

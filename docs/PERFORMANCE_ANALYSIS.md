...
| 2.1 | Lock-free MPSC mailboxes | **Shipped differently — optional bounded admission** | `src/runtime/mailbox.rs` uses lock-free `SegQueue` lanes with an atomic logical-message counter and optional capacity (`0` = unbounded), rather than `ArrayQueue`. Finite-capacity `Normal`/`Bulk` sends reserve a slot with CAS and are rejected on overflow; `System` messages bypass capacity. Most runtime spawn/recovery paths still pass `0`, so bounded admission exists as a mechanism but is not yet the default/source-configurable actor policy (#456). |

...
| GC | Per-actor ORCA: lazy bump-allocator heaps (16 KiB first-block capacity; 5 size classes, free lists), `local_count`/`foreign_count` per object; cross-actor sends bump `foreign_count` via `OrcaCoordinator`; deferred frees pumped every **256 scheduler ticks** and on run-queue drain | `runtime/heap.rs`, `runtime/gc.rs`, `runtime/mod.rs:1514` |
...
| Initial actor heap | 16 KiB first-block capacity, allocated on first small-object use | `runtime/actor.rs`, `runtime/heap.rs` |

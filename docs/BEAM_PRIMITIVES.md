...
7. ~~**No actor scheduling priority.**~~ **FIXED.** `Actor.priority: ActorPriority {High, Normal (default), Low}` set via `perform Actor.set_priority(0|1|2)`; the scheduler's global injector is split into three per-level queues drained High → Normal → Low (FIFO within a level). Message QoS is independent: `MessagePriority::{System, Normal, Bulk}` is enforced by distinct mailbox lanes drained System → Normal → Bulk, with FIFO preserved within each lane.

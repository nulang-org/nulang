...
1. ~~**Remote send drops the behavior name.**~~ **FIXED.** `Packet::ActorMessage` carries `behavior_name: String` on the wire; `process_network_packets` resolves it via `Runtime::behavior_id_for` against the target actor's behavior table and rejects unknown names without aliasing behavior 0 (`runtime/distributed.rs`, `runtime/network.rs`).

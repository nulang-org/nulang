...
| Mailbox | Lock-free concurrent `SegQueue` lanes plus a scheduler-local `VecDeque`, with optional capacity (`0` = unbounded). Bounded `Normal`/`Bulk` admission uses an atomic CAS reservation and rejects when full; `System` messages bypass the capacity. Selective receive stages messages without releasing their logical capacity slot until commit. `Message.payload` is `Arc<Vec<Value>>`. Most runtime spawn/recovery paths still construct actors with capacity `0`; source-level overflow policy is not yet configurable (#456). | `runtime/mailbox.rs`, `runtime/actor.rs` |
...
| Mailbox capacity | Optional per actor: `0` = unbounded; finite capacity rejects `Normal`/`Bulk` admission when full while `System` bypasses. Most runtime spawn/recovery paths currently pass `0`. | `runtime/mailbox.rs`, `runtime/actor.rs` |

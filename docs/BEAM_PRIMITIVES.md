...
| `{Name, Node} ! Message` | **IMPLEMENTED (runtime API)** | `Runtime::send_distributed(ActorAddress, behavior, args)` | `ActorAddress::Local`/`Remote` give location-transparent routing. Remote sends carry the behavior **name** on the wire; actors with declared behavior metadata reject unknown names on delivery. Metadata-free low-level runtime actors may admit raw mailbox traffic as inert id 0, where no handler exists. |
...
| BEAM Primitive | Nulang Status | Nulang Form | Rationale |
|----------------|---------------|-------------|-----------|
| `{Name, Node} ! Message` | **IMPLEMENTED (runtime API)** | `Runtime::send_distributed(ActorAddress, behavior, args)` | Location-transparent routing via `AddressResolver` + LRU `RemoteActorCache` (10,000 entries). Remote `ActorMessage` packets carry the behavior **name**; the receiver resolves it via `Runtime::behavior_id_for` on delivery and rejects unknown names without aliasing behavior 0. The `RSend` opcode (0xD2) is a no-op in the VM. |
...
| Scheduler | Sharded cooperative scheduling. Each live `Runtime` shard owns one scheduler thread and uses worker slot 0; `NULANG_SHARDS>1` runs shards in parallel over bounded cross-shard channels. Chase-Lev worker/steal APIs remain in `runtime/scheduler.rs` for alternate/future multi-worker callers, but peer stealing is not active in the current live per-shard `run_scheduler()` loop. Priority queues and scheduler profiling remain implemented. | `runtime/scheduler.rs`, `main.rs` |
...
| Persistence | `PersistenceStore` trait + `MemoryStore`, `JsonFileStore`, default-feature `LibsqlStore` (local SQLite/libSQL + remote Turso), plus optional `RocksDbStore` and `PostgresStore`; per-field `StateModel` (`Local`/`Durable`/`EventSourced`/`Crdt`); journal + snapshot recovery; `recover_actor` restores bytecode through `register_recovery_module`. Persisted strings are restored by content into the actor heap; unsupported pointer-like values still cannot be durably reconstructed. | `runtime/persistence.rs`, `runtime/mod.rs` |
...
A **16-byte versioned handshake** is exchanged immediately after TCP connect: `[magic "NUL0"][version u32][node_id u64]`. Bad magic or an incompatible wire version fails closed before framed traffic begins. With `TlsConfig::MutualTls`, TLS is established before this NUL0 handshake and node identity derives from the peer certificate fingerprint. Limits include `MAX_PACKET_LEN` 16 MiB and a bounded internal transport channel.

Packet discriminants currently span 0–16: `ActorMessage`, `Heartbeat`, `Ack`, `SpawnRequest`, `SpawnResponse`, `CrdtSync`, `Gossip`, `CrdtDeltaSync`, behavior fetch request/response, link/monitor/down, `CrdtOp`, actor migration, node goodbye, and shadow replication. Actor-message strings travel by content through a per-packet string table and are interned on the receiving scheduler thread; unsupported heap pointers, closures, actor refs, and `nil` are rejected before distributed send rather than serialized as ambiguous raw values.
...
1. ~~**Remote send drops the behavior name.**~~ **FIXED.** `Packet::ActorMessage` now carries `behavior_name: String` on the wire (length-prefixed UTF-8, replacing the `behavior_id: u16` field); `process_network_packets` resolves it via `Runtime::behavior_id_for` against the target actor's behavior table and rejects unknown names without aliasing behavior 0 (`runtime/distributed.rs`, `runtime/network.rs`).
2. ~~**Remote spawn is send-only.**~~ **FIXED (MVP).** `process_network_packets` now handles `Packet::SpawnRequest`: the receiving node spawns a fresh actor with the request's `initial_state` and registers the requested behavior — but only if that behavior was explicitly offered via `Runtime::register_spawnable_behavior` (MVP scope: remote spawn supports native behaviors the receiver opted into, not arbitrary or bytecode behaviors). Unknown names are answered with `SpawnResponse{success:false}` and no actor is created — the remote-spawn form of the same fail-closed rule used by actors with declared behavior metadata. The reply carries the real actor id; the requester collects it via `Runtime::take_spawn_response(request_id)` (recorded by the `SpawnResponse` arm of `process_network_packets`; the address returned by `spawn_on_node` is still a placeholder whose `actor_id` is the request id). `RSpawn` (0xD4) still returns `actor_ref(0)` (`vm.rs:1468`); `DistributedRuntimeImpl::spawn_on_node` still returns placeholder addresses.

...
- **Linux**, **macOS**, or **Windows x86_64**. Tagged release CI currently
  validates Linux x86_64/aarch64, macOS aarch64, and Windows x86_64. Other
  source-build combinations may work but are not release-tested.
...
Use the tagged Windows x86_64 release for the lowest-friction install. For
source builds, use the pinned Rust toolchain and a compatible C/C++ build
environment; optional native dependencies may impose additional platform
requirements. WSL remains a valid alternative when you want the Linux build
environment.

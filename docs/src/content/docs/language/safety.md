...
- **Pattern matching**: closed variants and `Bool` receive conservative compile-time coverage warnings for provably missing or redundant arms; unsupported/infinite domains retain the runtime `non-exhaustive match` fallback. Use `--deny-warnings` for opt-in strict enforcement.
...
- **Mailbox isolation**: each actor has a private mailbox. Delivery can fail—for example, a bounded mailbox can overflow or a target can disappear—and those failures use explicit runtime rejection/backpressure/DLQ paths rather than silently executing another handler.
...
| **Pattern matching** | Typed; conservative finite-domain coverage warnings with runtime fallback | Non-exhaustive by default |

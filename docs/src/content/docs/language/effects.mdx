...
| `Process` | `run` | Trusted/standalone shell command execution |
| `Debug` | `inspect` | Labeled value inspection |

> `Process.run` is intentionally not dispatched by actor-backed production runtimes. The standalone VM implementation uses `/bin/sh -c`, which inherits host OS authority and is therefore unsuitable for untrusted actor execution until a separate isolated process sandbox with explicit resource and filesystem/network policy exists.

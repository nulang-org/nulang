...
Use the project build command when another runtime or platform needs a portable
package artifact:

```bash
nulang build --target wasm
# .nula/dist/<name>.wasm
# .nula/dist/<name>.behavior.json
```

This is the canonical **portable package artifact** path. It performs the same
dependency resolution, language-version checks, module-path setup, and declared
capability forwarding as the normal package build before invoking the WASM
backend.

The adjacent `*.behavior.json` file conforms to
`spec/behavior/v0alpha1.schema.json` and is bound to the exact emitted WASM
bytes. Normative fields include compiler-binary/source/dependency digests,
normalized actor records, unique effect records, required authority categories,
durability summaries, and conservative replay metadata.

The compiler also contributes a conservative semantic inventory covering every
declared module function, actor behavior, and workflow step/compensation. Rich
per-subject evidence is retained under the advisory
`extensions["nulang.org/compiler-semantics"]` namespace rather than changing
the public v0alpha1 field shapes. Actor protocol IDs reuse the canonical
structural protocol hash and are emitted only when parameter types, return
type, and a closed effect contract are explicit; otherwise the compiler
extension records an incomplete status and the public actor omits `protocol`.

Package-declared capabilities remain advisory configuration under
`extensions["nulang.org/package-capabilities"]`. The normative `authority`
array is populated only from compiler-inferred requirements. State-schema and
migration identities are not emitted yet; durability currently reports only
the compiler-known persistence class.

For a Wasmtime-precompiled sidecar:

```bash
nulang build --target cwasm
# .nula/dist/<name>.wasm
# .nula/dist/<name>.cwasm
```

The historical command remains available as a compatibility alias:

```bash
nulang nula build-wasm
# equivalent to: nulang build --target cwasm
```

:::caution
The WASM targets require a `nulang` binary built with the `wasm-backend`
cargo feature. Without it the command fails with
`wasm backend not compiled in (enable 'wasm-backend' feature)`.
:::

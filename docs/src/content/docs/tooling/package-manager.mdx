...
The adjacent `*.behavior.json` file is the compiler/package contract defined
by RFC 0020. It is bound to the exact emitted WASM bytes with a BLAKE3 digest
and records package/language/compiler identity plus source and lockfile
provenance.

The compiler also contributes a conservative semantic inventory covering every
declared module function, actor behavior, and workflow step/compensation.
Effect entries contain compiler-inferred body effects, and resource-gated
effects derive coarse required authority categories (`fs`, `net`, `os`).
Actor protocol IDs reuse the canonical structural protocol hash and are emitted
only when the behavior signatures are stable enough to hash; otherwise each
actor carries an explicit incomplete status instead of a guessed identity.

Package-declared capabilities remain separately labelled as declarations.
Interfaces, durable-state schemas/migrations, and replay classifications are
still marked `not-emitted` until their compiler-owned contracts land.

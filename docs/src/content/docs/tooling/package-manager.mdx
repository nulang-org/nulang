...
RFC 0020 Behavior Manifest. It is bound to the exact emitted WASM bytes with a
BLAKE3 digest and records package/language/compiler identity plus source and
lockfile provenance. Until compiler semantic inventories land, its
`completeness` object explicitly marks effects, actor protocols, durability,
and replay metadata as `not-emitted`; package capabilities are labelled
`declared-not-inferred` rather than being presented as compiler-derived
authority.

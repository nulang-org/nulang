...
The adjacent `*.behavior.json` file is the first implementation slice of the
RFC 0020 Behavior Manifest and conforms to
`spec/behavior/v0alpha1.schema.json`. It is bound to the exact emitted WASM
bytes with a BLAKE3 digest and records package/language/compiler identity,
compiler-binary digest, and source/dependency provenance.

Until compiler semantic inventories land, the namespaced advisory
`extensions["nulang.org/completeness"]` record marks effects, actor protocols,
authority, durability, and replay metadata as `not-emitted`. Package-declared
capabilities are preserved separately under
`extensions["nulang.org/package-capabilities"]`; they are configuration, not
compiler-derived authority and therefore do not populate the normative
`authority` array.

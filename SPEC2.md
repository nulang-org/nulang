...
- Migration contracts (RFC 0008): `version: N`, `migration from N to M { ... }` blocks inside entities. **Experimental, not Stable.** The compiler now validates positive versions, contiguous one-version migration edges, chain termination, migration parameter binding, and migration purity. Durable snapshots and event-sourced log entries carry compiler-owned schema identity/version metadata (legacy records default to v1), and restart/recovery/hydration/migration paths fail closed on incompatible persisted versions before state becomes observable. **State-only runtime migration is implemented for the registered-module restart path, with deliberately narrow scope:** MIR lowering emits canonical migration topology and compiler-private state-transform functions; `recover_actor` can execute an adjacent state-migration chain in a fresh VM bound directly to an unpublished actor, then persist the upgraded snapshot at the original durable sequence before normal actor publication. The migration VM denies actor messaging/spawn, external effects/FFI/async work, event emission, mailbox/signal operations, and distributed operations. This executor currently accepts ordinary `state durable` snapshots only and fails closed for workflows, event-sourced fields/history, CRDT state, declared event transforms, old-schema journal entries newer than the snapshot, missing durable fields, and unsupported heap-backed durable values. Event-history migration, CRDT migration, workflow migration, and cross-node old-schema actor handoff remain unimplemented. Registered virtual actors now use the same isolated state-migration + revision-CAS preparation path before hydration publication. Current-schema event-sourced recovery can additionally execute compiler-private replay-safe `apply` projections in an isolated VM: per-field event rows are grouped by sequence into one logical event, only `event_sourced` state is accessible, and unsafe/legacy projections retain the recorded post-apply-value compatibility path. Historical-version event transforms are still not executed; RFC 0008 event migration will feed transformed events into this current-schema projection stage. Registered-module state migration commits are fenced by `PersistenceStore::compare_and_swap_snapshot` over `(actor_id, sequence, schema_owner, schema_version)`: Memory/libSQL/Postgres and the exclusive-writer RocksDB backend implement the fence; backends without a real concurrent conditional write (currently `JsonFileStore`) report `Unsupported`, and recovery refuses the unsafe upgrade. A CAS loser reloads the winner and may continue only if that snapshot is already at the artifact's current schema. See `conformance/behavior/migration_*.nula` and RFC 0008.

...
- Migration contracts (RFC 0008): `version: N`, `migration from N to M { ... }` blocks inside entities. **Experimental, not Stable.** The compiler validates positive versions, contiguous adjacent migration chains, parameter/catch-all binding, and migration purity. Durable snapshots and event rows carry compiler-owned schema owner/version metadata, with legacy records treated as v1. Registered-module restart and virtual-actor hydration use one revision-CAS-fenced snapshot preparation path: state transforms execute in a fresh unpublished VM, may read fields physically present in the old snapshot and write current durable fields, and cannot access event-sourced/CRDT state, actor messaging, external effects, FFI, async operations, mailboxes/signals, or distribution. Historical event migration is also executable: persistence rows at one sequence are validated and collapsed into one logical event, old events advance through each adjacent migration contract in memory, named arms execute compiler-private `Entity.$migration_event_V_W_Event` functions whose `emit` operations are captured into the replay stream, `other => other` performs declarative pass-through, and zero/one/many replacement events are supported without rewriting the persistent journal. Once events reach the current schema, replay-safe compiler-private `apply` projections reconstruct current `event_sourced` state; historical histories require executable replay and therefore fail closed if a transform or current apply projection is missing/unsafe, while already-current legacy artifacts may still use the recorded post-apply-value compatibility path. **Known limitation:** RFC 0008 says `self` inside a migration refers to old-version entity state. State-migration bodies have a real old-snapshot view, but historical event-transform arms do not yet have a versioned old-state view; any event-transform `self.field` access therefore fails closed rather than observing already-migrated state. CRDT migration, workflow-history migration, cross-node old-schema actor handoff, and a complete historical event-type registry across multi-version chains remain unimplemented. Migration topology identity excludes artifact-local state/event function-table indices so code layout changes do not change the semantic migration digest. See `conformance/behavior/migration_*.nula` and RFC 0008.

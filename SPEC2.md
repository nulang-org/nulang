...
- Migration contracts (RFC 0008): `version: N`, `migration from N to M { ... }` blocks inside entities. **Experimental, not Stable.** The compiler now validates positive versions, contiguous one-version migration edges, chain termination, migration parameter binding, and migration purity. Durable snapshots and event-sourced log entries carry compiler-owned schema identity/version metadata (legacy records default to v1), and restart/recovery/hydration/migration paths fail closed on incompatible persisted versions before state becomes observable. **Runtime migration execution is still not implemented:** MIR lowering emits a canonical, validated declarative migration topology into `ActorMeta`; state migration bodies are additionally lowered into compiler-private function-table entries and each state transition is bound to its artifact-local function index. Recovery validates the binding against the canonical generated function identity before activation. Event migration bodies remain metadata-only, and recovery still does not invoke state transforms or rewrite event history, so version mismatches are rejected rather than transformed. Until the deterministic executor/commit path lands, a valid migration contract proves source validity, artifact topology, and state-transform code binding but does not make an old durable instance upgradeable at runtime. See `conformance/behavior/migration_*.nula` and RFC 0008.

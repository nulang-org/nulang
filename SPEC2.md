...
**Runtime handler resolution.** Whether an enclosing handler exists for a performed effect is decided dynamically. An unhandled effect is a runtime `EffectError` (`Unhandled effect: 'Name'`), so programs that perform effects must install handlers (or use the runtime-backed operations) to avoid failing at runtime. Captured continuations are affine: both heap-captured continuations and optimized single-shot state are consumed by `Resume`, and a second resume traps deterministically. The formal model proves pushed-handler dispatch, that a bare `perform` cannot type with an empty effect row, and the affine continuation transition; the stronger whole-program handler-stack safety theorem remains open. Static exhaustiveness checking of effect handlers is still planned. Ordinary pattern `match` expressions separately use a typed Maranget-style usefulness matrix: `W0201` reports uncovered witnesses and `W0202` reports subsumed arms; guarded arms do not contribute to total coverage.

...
- Migration contracts (RFC 0008): `version: N`, `migration from N to M { ... }` blocks inside entities. **Experimental, not Stable.** The compiler now validates positive versions, contiguous one-version migration edges, chain termination, migration parameter binding, and migration purity. Durable snapshots and event-sourced log entries carry compiler-owned schema identity/version metadata (legacy records default to v1), and restart/recovery/hydration/migration paths fail closed on incompatible persisted versions before state becomes observable. **Runtime migration execution is still not implemented:** MIR lowering now emits a canonical, validated declarative migration topology into `ActorMeta`, and recovery validates/plans that topology before failing closed on an old schema. Executable state/event migration bodies are not yet lowered/bound to the manifest, so version mismatches are still rejected rather than transformed. Until that deterministic executor lands, a valid migration contract proves both source validity and artifact topology but does not make an old durable instance upgradeable at runtime. See `conformance/behavior/migration_*.nula` and RFC 0008.

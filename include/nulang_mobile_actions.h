...
/**
 * Invoke one nulang-action-invoke/1 request.
 *
 * Returns runtime-owned nulang-action-result/1 JSON, or NULL on failure. The
 * returned pointer remains valid until the next invocation on this runtime or
 * until the runtime is freed. Calls on one runtime instance must be serialized.
 */

...
- **Error handling** — `Result[T, E]` values with `?` propagation and readable `throws E` signature sugar; algebraic effects handle checked side effects and actor supervision handles faults. Legacy `catch`/`fail` forms are being retired under RFC 0015.

...
//!
//! Current 1.x compatibility keeps coverage findings as warnings rather than new
//! unconditional hard errors. RFC 0021 permits a future explicit semantic
//! transition after evidence, but that transition must be versioned rather than
//! smuggled into a diagnostic patch. Projects can opt into strict matching now
//! with the existing `--deny-warnings` policy.
...
fn warnings_from_report(report: CoverageReport, span: Span) -> Vec<NuWarning> {
    ...
    if !report.missing.is_empty() {
        ...
        warnings.push(NuWarning {
            ...
            span,
            help: Some(format!(
                "add arm{} for {missing}; without one, an uncovered value retains the current runtime non-exhaustive-match behavior",

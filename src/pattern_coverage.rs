...
//!
//! Nulang 1.x preserves the validity and runtime behavior of existing `match`
//! programs while the Stable Core evolves under the RFC process, so coverage
//! findings are warnings rather than new hard type errors. Projects that want
//! strict matching can opt into the existing `--deny-warnings` policy.
...
fn warnings_from_report(report: CoverageReport, span: Span) -> Vec<NuWarning> {
    ...
    if !report.missing.is_empty() {
        ...
        warnings.push(NuWarning {
            ...
            span,
            help: Some(format!(
                "add arm{} for {missing}; without one, an uncovered value keeps the runtime non-exhaustive-match behavior",

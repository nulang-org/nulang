...
pub trait JitBackend {
    ...
    ///
    /// Deliberately receives no CodeModule reference: the VM may move the JIT
    /// backend out of itself and drop every module borrow before entering
    /// native code, which is required when runtime helpers can re-enter the VM.
    fn execute_compiled(
        &mut self,
        module_idx: usize,
        pc: usize,

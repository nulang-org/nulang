...
"#;
    ...
    fn test_workflow_signal_wait_and_resume_after_restart() {
        ...
        rt2.borrow_mut().signal_workflow(actor_id, "go", None).unwrap();
    ...
    fn test_workflow_step_waits_on_two_sequential_signals() {
        ...
        rt.borrow_mut().signal_workflow(actor_id, "first", None).unwrap();
        {
            let rt = rt.borrow();
            let actor = rt.actors.get(&actor_id).unwrap();
            assert_eq!(
        ...
        rt.borrow_mut().signal_workflow(actor_id, "second", None).unwrap();
        {
            let rt = rt.borrow();
            let actor = rt.actors.get(&actor_id).unwrap();
            assert_eq!(
    ...
    fn test_workflow_parallel_branches_and_recovery() {
        ...
        rt2.borrow_mut().signal_workflow(actor_id, "continue", None).unwrap();
        rt2.borrow_mut().run_scheduler();

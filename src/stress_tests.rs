...
fn stress_scheduler_with_mixed_workload() {
    ...
    rt.actors.get_mut(&cpu_actor).unwrap().register_behavior("compute", |_actor, _args| {});
    rt.actors.get_mut(&io_actor).unwrap().register_behavior("io_op", |_actor, _args| {});
    rt.actors.get_mut(&sink).unwrap().register_behavior("collect", |_actor, _args| {});
...
fn stress_reduction_quota_fairness() {
    ...
    rt.actors.get_mut(&actor_a).unwrap().register_behavior("work", |_actor, _args| {});
    rt.actors.get_mut(&actor_b).unwrap().register_behavior("work", |_actor, _args| {});
...
fn stress_effect_resume_after_mailbox_pressure() {
    ...
    rt.actors.get_mut(&effect_actor).unwrap().register_behavior("start_effect", |_actor, _args| {});
    rt.actors.get_mut(&effect_actor).unwrap().register_behavior("flood", |_actor, _args| {});
...
fn stress_gc_foreign_ref_churn() {
    ...
    rt.actors.get_mut(&target).unwrap().register_behavior("ref", |_actor, _args| {});
...
fn stress_distribution_local_fallback_when_disabled() {
    ...
    rt.actors.get_mut(&actor).unwrap().register_behavior("ping", |_actor, _args| {});
...
fn stress_reduction_yield_under_pressure() {
    ...
    for i in 0..50 {
        ...
        rt.actors.get_mut(&id).unwrap().register_behavior("work", |_actor, _args| {});
        actors.push(id);
    }
...
fn stress_actor_ping_pong_messaging() {
    ...
    rt.actors.get_mut(&pinger).unwrap().register_behavior("pong", |_actor, _args| {});

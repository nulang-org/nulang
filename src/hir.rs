...
pub struct ActorDef {
    ...
    /// Typed/lowered state migration bodies.
    pub migration_state_bodies: Vec<MigrationStateBody>,
    /// Named RFC 0008 event migration arms lowered as compiler-private
    /// transforms. Catch-all `other => other` remains declarative pass-through
    /// metadata and is not represented as executable bytecode.
    pub migration_event_bodies: Vec<MigrationEventBody>,
...
pub struct MigrationEventBody {
    ...
    pub to_version: u32,
    pub event: String,
    /// Historical payload parameters are intentionally dynamically typed at
...
mod tests {
    ...
    fn test_actor_def_new() {
        let def = ActorDef {
            ...
            events: vec![],
            apply_handlers: vec![],
            apply_handler_bodies: vec![],
            version: 1,
            migrations: vec![],
            migration_state_bodies: vec![],
            migration_event_bodies: vec![],
            is_workflow: false,

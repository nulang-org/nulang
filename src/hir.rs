...
//!   - source-only sugar such as agents and workflows has already lowered to
//!     canonical actor semantics;
//!   - compile-time-only declarations are absent;
...
pub struct ActorDef {
    ...
    /// Canonical compiler-owned actor protocol identity when every behavior
    /// signature is explicit and structurally stable enough to fingerprint.
    ///
    /// Absence is fail-closed: downstream manifests/runtime code must not
    /// invent a protocol identity from incomplete HIR types.

...
use crate::protocol::{ProtocolMember, ProtocolSchema, ProtocolTypeId};
...
impl BehaviorManifest {
    ...
    pub fn from_checked_module(
        ...
        let replay = effect_decls.iter().map(replay_decl).collect();
        let authority = authority_requirements(&effects);
        let actors = Self::collect_actor_contracts(decls);
        ...
        Ok(Self {
            ...
            // Interfaces and state schemas remain empty until their compiler
            // source of truth is authoritative. Actor declarations are safe to
            // emit now because names/durability come directly from the checked
            // AST and protocol identities reuse the canonical NTIR-backed
            // protocol hash in crate::protocol.
            interfaces: Vec::new(),
            actors,
    ...
    fn collect_actor_contracts(decls: &[Decl]) -> Vec<ActorDecl> {
        let mut actors = Vec::new();
        Self::collect_actor_contracts_into(decls, &mut actors);
        actors.sort_by(|a, b| a.name.cmp(&b.name));
        actors
    }

    fn collect_actor_contracts_into(decls: &[Decl], out: &mut Vec<ActorDecl>) {
        for decl in decls {
            match decl {
                Decl::Actor {
                    name,
                    ...
                    protocol: Self::actor_protocol_id(name, behaviors),
                    state_schema: None,
                }),
                Decl::StateMachine {
                    name,
                    ...
                    let desugared = crate::ast::desugar_state_machine(
                        name,
                        states,
                        events,
                        entry_hooks,
                    ...
                    Self::collect_actor_contracts_into(std::slice::from_ref(&desugared), out);
                }
                Decl::Module { decls, .. } => Self::collect_actor_contracts_into(decls, out),
                _ => {}
            }
        }
    }

    fn actor_protocol_id(name: &str, behaviors: &[crate::ast::Behavior]) -> Option<String> {
        let mut members = Vec::with_capacity(behaviors.len());

        for behavior in behaviors {
            // Fail closed: a deployment-visible protocol identity is emitted only
            // when every parameter and the response type are explicit. Inferring a
            // wire contract from implementation bodies would make refactors change
            // compatibility accidentally.
            let params = behavior
                .params
                .iter()
                .map(|param| param.ty.as_ref().map(ProtocolTypeId::from_type))
                .collect::<Option<Vec<_>>>()?;
            let response = ProtocolTypeId::from_type(behavior.ret_type.as_ref()?);
        ...
        let schema = ProtocolSchema::new(name, members).ok()?;
        Some(format!("nulang.protocol/v1:{}", schema.id()))
    }
...
fn replay_decl(effect: &EffectDecl) -> ReplayDecl {
    let class = match effect.replay {
        EffectReplay::None => ReplayClass::Pure,
        EffectReplay::Safe => match effect.class {
            EffectClass::Pure => ReplayClass::Pure,
            EffectClass::Local => ReplayClass::LocalReplaySafe,
            // If a future externally visible effect is classified Safe, keep
            // deployment metadata conservative rather than silently treating
            // it as local-only.
            EffectClass::External => ReplayClass::JournalResult,
        },

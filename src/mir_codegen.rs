...
/// Build in-memory entry representation facts for the tiered JIT.
///
/// Only call-staging argument registers are seeded. Locals are not trusted
/// globally because later operations can yield values such as nil despite a
/// source-level numeric type. The ordinary bytecode must-analysis propagates
/// these facts through the prologue, and JitSession guards any compiled region
/// whose analysis depends on this seed before native execution.
...
impl MirCodegen {
    ...
    fn compile_function(
        ...
        // Source compilation can give the tiered JIT primitive argument
        // representation facts that bytecode alone cannot recover. The table
        // is serde-skipped and therefore never becomes an artifact trust
        // boundary; native entry validates the corresponding live tags.

...
use crate::bytecode::{
    CodeModule, Constant, DebugFunctionInfo, EffectSiteMetadata, ForeignFunctionDef, HandlerBinding,
    HandlerTable, Instruction, OpCode,
};
use crate::mir;
    ...
    pub fn compile_module(&mut self, mir: &mut mir::Module) -> NuResult<&CodeModule> {
        ...
        for (idx, func) in mir.functions.iter().enumerate() {
            let sites: Vec<_> = semantic_effect_sites
                .iter()
                .filter(|site| {
                    site.owner_kind == EffectSiteOwnerKind::Function
                        && site.owner_name == func.name
                })
                .cloned()
                .collect();
        ...
        for func in &mir.behaviors {
            let sites: Vec<_> = semantic_effect_sites
                .iter()
                .filter(|site| {
                    site.owner_kind == EffectSiteOwnerKind::Behavior
                        && site.owner_name == func.name
                })
                .cloned()
                .collect();

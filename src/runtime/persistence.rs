...
pub struct ActorSnapshot {
    ...
    /// Canonical declared actor schema (ActorMeta.name) represented by this
    /// snapshot. Legacy snapshots omit it and are accepted only when the
    /// loaded module has exactly one unambiguous actor schema.
    #[serde(default)]

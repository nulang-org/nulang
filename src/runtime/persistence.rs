...
pub struct ActorSnapshot {
    ...
    /// durable history. Legacy snapshots omit it and remain explicitly
    /// unverified; a present identity must match recovery definition code.
    #[serde(default)]
    pub semantic_id: Option<String>,
    /// Exact compiler/backend artifact that produced this checkpoint, when
    /// known. Artifact identity may change across a semantics-preserving
    /// rebuild while `semantic_id` remains stable.
    #[serde(default)]
    ...
    fn ensure_tables(&self) -> io::Result<()> {
        ...
        self.rt.block_on(async {
            ...
            let _ = conn
                ...
                .await;
            // Strong semantic provenance was added later. Existing rows remain
            // NULL and are treated as explicit legacy/unverified snapshots.
            let _ = conn
                .execute("ALTER TABLE snapshots ADD COLUMN semantic_id TEXT", ())
                .await;
            // Exact artifact provenance is additive; legacy rows remain NULL.
            let _ = conn

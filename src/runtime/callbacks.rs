...
fn required_host_authority(
    ...
    let grant = match (effect_name, op_name) {
        ...
        // a direct FS/Env/Net/FFI access does. They intentionally resolve to
        // `None` (no grant required), preserving the ambient contract.
        _ => return Ok(None),
    };
    Ok(Some(grant))
}
    ...
    fn migrate(&mut self, actor_id: u64, target_node_id: u64) {
        unsafe {
            ...
            let (snapshot_json, nbc_bytes) = {
                ...
                let snapshot_json =
                    match crate::persistence_schema::encode_record(&snapshot, actor.schema_version)
                    {

...
impl TraceContext {
    ...
    /// context.
    ///
    /// Boundary and delivery semantics are explicit inputs rather than being
    /// inferred from the effect name. The same effect can have different
    /// replay guarantees depending on the installed adapter or backend.
    /// `operation_id` should contain a stable replay/idempotency identity when
    /// one exists; callers should leave it absent rather than invent one.
    ...
    /// Enter an effect span only when TRACE-level tracing is enabled.

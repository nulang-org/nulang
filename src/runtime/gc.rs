...
pub struct GcStats {
    ...
    /// Successful compiler-proven local-to-foreign ownership handoffs.
    pub ownership_handoffs: u64,
    /// Header refcount mutations avoided relative to the conservative
    /// in-flight-send + receiver-hold protocol.
...
impl OrcaGc {
    ...
    /// Convert one proven unique local ownership token directly into a
    /// foreign receiver hold. This is valid only while owner and receiver
    /// execute on the same scheduler shard; callers must still record the hold
    /// on the receiver after mailbox admission succeeds.
    ///
    /// # Safety
    /// payload_ptr must be a live object owned by this actor and the caller
    /// must have compiler proof that the consumed local token has no other use.
    ...
    /// Undo a handoff when mailbox admission fails. Statistics are restored
    /// too so observability reflects successful handoffs rather than attempts.
    ///
    /// # Safety
    /// payload_ptr must be the same live owner object successfully handed
    /// off above and not yet adopted elsewhere.

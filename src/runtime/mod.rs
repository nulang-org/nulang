...
impl Runtime {
    ...
    /// Enqueue an actor at its current priority, at most once.
    ///
    /// The owning shard is the sole writer of `run_state`. A Queued actor is
    /// already represented by one ready token; a Running actor is reconsidered
    /// exactly once by `finish_actor_turn`. Sends to either state only append
    /// mailbox work and do not inject duplicate scheduler entries.

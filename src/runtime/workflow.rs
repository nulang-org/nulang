    ...
    if let Some(actor) = rt.actors.get_mut(&actor_id) {
        ...
        if !is_workflow && !event_sourced_names.is_empty() {
            ...
            for name in &event_sourced_names {
                // Capture the field's current value AFTER any source-level
                // apply handler has run. Event emission itself must never
                // invent a domain-state mutation; apply handlers (or explicit
                // behavior code) are the sole authority for state evolution.
                // Persisting the post-apply value lets recovery reconstruct
                // current state without re-executing bytecode.
...
pub(crate) fn signal_workflow(
    ...
    if let Err(error) = append_signal_received(rt, actor_id, name, payload.clone()) {
        tracing::warn!(
            ...
            signal = name,
            %error,
            "nulang-persist: refusing to deliver workflow signal without durable commit"
        );
        return;
    }

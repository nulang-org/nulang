...
//!
//! Recovery must never infer an actor schema from behavior-table position or
...
mod tests {
    ...
    fn unknown_schema_fails_closed() {
        ...
        assert!(matches!(
            resolve_snapshot_actor_meta(&module, Some("Missing")),
            Err(SnapshotSchemaError::UnknownPersistedSchema { .. })
        ));
    }
    ...
    fn legacy_multi_schema_snapshot_never_guesses_owner() {
        ...
        assert!(matches!(
            resolve_snapshot_actor_meta(&module, None),
            Err(SnapshotSchemaError::AmbiguousLegacySnapshot { .. })
        ));
    }
    ...
    #[test]
    fn expected_schema_rejects_other_valid_schema() {
        let module = compile(
            r#"
        ...
        assert!(matches!(
            resolve_expected_snapshot_actor_meta(&module, Some("Other"), "Counter"),
            Err(SnapshotSchemaError::UnexpectedPersistedSchema { .. })
        ));
    }
}

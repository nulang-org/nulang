...
/// Invoke one versioned client-action request.
///
/// Returns a NUL-terminated `nulang-action-result/1` JSON string owned by the
/// runtime, or null on failure. The result pointer remains valid until the next
/// invocation on this runtime or until the runtime is freed.
///
/// # Safety
...
pub unsafe extern "C" fn nulang_mobile_action_runtime_invoke(
    ...
    let result_json = match serde_json::to_string(&result) {
        ...
        Err(error) => {
            runtime.set_error(format!("failed to encode client action result: {error}"));
            return std::ptr::null();
        }
    };
...
fn save(request: String) -> String {
    "{\"protocol\":\"nulang-action-result/1\",\"correlation_id\":\"corr-ffi\",\"messages\":[]}"
}
"#;
        ...
        let function_index = module.function_offset_by_name("save").expect("save") as u32;
    ...
    fn c_boundary_invokes_only_authorized_action() {
        ...
        let request = CString::new(
            r#"{"protocol":"nulang-action-invoke/1","handler":"save","correlation_id":"corr-ffi","idempotency_key":"save:corr-ffi","form":{},"signals":{}}"#,
        )
        .unwrap();
        ...
        assert_eq!(json["protocol"], "nulang-action-result/1");
        assert_eq!(json["correlation_id"], "corr-ffi");

        let unauthorized = CString::new(
            r#"{"protocol":"nulang-action-invoke/1","handler":"secret","correlation_id":"corr-ffi","idempotency_key":"secret:corr-ffi","form":{},"signals":{}}"#,
        )
        .unwrap();

...
pub fn build_client_action_metadata(
    ...
    for handler in handlers {
        ...
        let function_index = module.function_offset_by_name(&handler).ok_or_else(|| {
        ...
        // Until the semantic UI compiler grows opaque action IDs, the handler
        // identity is the stable action ID. The artifact format keeps the two
        // fields distinct so a later compiler can change that mapping without
        // changing the runtime authorization model.

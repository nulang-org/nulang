...
/// Runtime tag assumptions introduced specifically by compiler-owned entry
/// seeds. Bytecode-only type proofs remain guard-free.
...
struct CompiledRegion {
    ...
    tier2_terminal: bool,
    /// Only present when compiler-owned entry facts contributed to the proof.
...
pub struct JitSession {
    ...
    /// dense post-compilation tiering state.
...
impl JitSession {
    ...
    fn store_compiled(
        ...
        match row[offset].as_mut() {
            Some(region) => {
                // Tier-2 recompilation replaces executable code in place while
                // preserving the region's post-compilation state.
                region.ptr = ptr;
    ...
    /// function-table index). A function with `false` here is safe to call
    /// from JIT-compiled code because its full direct-call closure is proven
    /// non-suspending.
    #[allow(dead_code)]
    fn may_suspend_for(
        &mut self,
    ...
    ///
    /// Tier-1 already performs static type inference. An untyped region cannot
    /// become typed merely by executing more often with the current JIT, so it
    /// becomes terminal at the threshold. Typed regions get one real SIMD
    /// replacement attempt; success or static rejection is terminal as well.
    fn record_tier2_and_maybe_promote(
        &mut self,
        module_idx: usize,
        pc: usize,
        module: &crate::bytecode::CodeModule,
        ...
        // SIMD eligibility is static for this bytecode/target. Retrying the
        // same analysis every 10k entries cannot reveal new facts.
        if let Some(region) = self.compiled_entry_mut(module_idx, pc) {
            region.tier2_terminal = true;
        }
    }

    /// Reset execution counters without changing terminal decisions.
    ...
    ///
    /// Unlike `compile_region_simd`, this deliberately bypasses the
    /// already-compiled cache check so Tier-2 can replace Tier-1 code. Failure
    /// leaves the existing compiled pointer untouched.
    unsafe fn promote_region_simd(
        ...
        // Tier-1 already declared a symbol for this bytecode start; promotion
        // needs a distinct name before replacing the cached pointer.
...
///
/// - may-suspend: reverse reachability from directly unsafe functions;
/// - recursive: strongly-connected components of the direct-call graph.
///
/// Function bytecode is scanned once. The graph projections are then computed
/// in O(V + E), replacing repeated body rescans and Floyd-Warshall closure.
fn compute_module_jit_analysis(module: &crate::bytecode::CodeModule) -> ModuleJitAnalysis {
        ...
        for pc in start..end {
            ...
            if matches!(op, OpCode::Call | OpCode::ClosureCall) {
                match direct_call_target(module, pc, start) {
                    ...
                    _ => {
                        // Indirect/malformed calls are conservatively unsafe
                        // for the re-entrant native-call helper.
                        may_suspend[i] = true;
                    }
                }
    ...
    // Reverse edges let suspension propagate to callers with one worklist.
...
/// Mark functions that belong to a direct-call recursion cycle using an
/// iterative Kosaraju traversal. Iterative stacks keep compiler input size off
/// the host Rust call stack.
...
/// Compatibility projections retained for focused unit tests.
#[allow(dead_code)]

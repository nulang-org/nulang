...
pub struct JitSession {
    ...
    /// intentionally not inserted into `compiled`: direct-call thunks are
    /// entered only from an already-native caller.
    native_leafs: FxHashMap<(usize, usize), NativeCalleeThunk>,
    /// Static native-callee analyses/compilations that failed; retrying them
    /// cannot become useful until the module changes.

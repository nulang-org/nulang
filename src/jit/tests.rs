...
fn test_compiled_direct_call_deopts_when_live_target_changed() {
    ...
    // The compile-time target says function 0, but the live register says 1.
    // The region must deopt at pc 0 rather than invoking the stale target.
...
fn test_jit_direct_call_loop_tiers_up() {
    // A hot loop calling a tiny straight-line non-suspending leaf must fold
    // the call into the compiled region and compile the callee as a separate
    // native leaf thunk. The thunk runs directly from the caller's native code
    // while register save/restore preserves the interpreter's frame isolation.
    // If the call were NOT folded

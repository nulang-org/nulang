...
fn test_typed_basic_block_leaders_coalesce_straight_line_code() {
    ...
    assert!(
        !leaders.contains(&1),
        "straight-line instructions must share their predecessor's CLIF block"
    );
}
...
fn test_tier2_counter_increments_and_becomes_terminal() {
    ...
    // Counter reset must not demote a terminal region.
...
fn test_tier2_simd_promotion_does_not_short_circuit_on_cached_region() {
    ...
    // Tier-2 replacement must analyze the region rather than treating the
    // cached Tier-1 pointer as a successful promotion.

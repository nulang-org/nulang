...
/// Retain a child reference for storage inside a local container.
unsafe fn retain_container_obj(ptr: *mut u8) -> bool {
    if let Some(ok) = try_with_callbacks(|cb| cb.retain_container_child(ptr)) {
        return ok;
    }
    if !ptr.is_null() {
        let header = &mut *ActorHeap::header_of(ptr);
...
/// Release a child reference previously stored inside a local container.
unsafe fn release_container_obj(ptr: *mut u8) {
    if try_with_callbacks(|cb| {
        cb.release_container_child(ptr);
        true
    })
    .is_some()
    {
...
pub unsafe extern "C" fn nulang_obj_set(obj: u64, idx: u64, val: u64) {
    ...
    if header.type_tag == HeapTypeTag::ArrayView {
        ...
        if let Some(old_backing) = old_backing {
            release_container_obj(old_backing);
        }
        return;
    }

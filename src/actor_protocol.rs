...
struct ActorProtocol {
    ...
    // Phase 1 resolves actor identities by short name. If more than one
    // declaration shares that name (for example in nested modules), choosing
    // either protocol would be unsound. Mark the name ambiguous and preserve
    // the dynamic compatibility fallback until qualified/structural protocol
    // identities land.
    ambiguous: bool,
}
    ...
    for decl in decls {
        match decl {
            Decl::Actor {
                ...
                insert_protocol(out, name, protocol_from_behaviors(behaviors, state_fields));
            }
            Decl::StateMachine {
                name,
                states,
...
fn insert_protocol(
    out: &mut FxHashMap<String, ActorProtocol>,
    name: &str,
    ...
    if let Some(existing) = out.get_mut(name) {
        ...
        out.insert(name.to_string(), protocol);
    }
}
...
fn set_actor_binding(
    ...
    if let Some(actor_name) = actor_name.filter(|name| {
        protocols
            .get(name)
            .is_some_and(|protocol| !protocol.ambiguous)
    }) {
...
mod tests {
    ...
    #[test]
    fn duplicate_short_actor_names_across_modules_do_not_select_arbitrary_protocol() {
        let ast = parse(
            r#"
            module First {
                actor Worker { behavior first() { nil } }
                fn run() {
                    let worker = spawn Worker {} in send worker first()
                }
            }

            module Second {
                actor Worker { behavior second() { nil } }
            }
            "#,
        );

        let result = annotate_module(&ast);
        assert!(
            result.is_ok(),
            "ambiguous short actor names must fall back to dynamic checking instead of selecting another module's protocol: {:?}",
            result.err()
        );
    }
}

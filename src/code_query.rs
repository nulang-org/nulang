...
fn cmd_symbols(args: &[String]) -> NuResult<()> {
    ...
    let mut report = analyze_for_cli(Path::new(file), "symbols", json)?;
...
fn cmd_symbol(args: &[String]) -> NuResult<()> {
    ...
    let mut report = analyze_for_cli(Path::new(positional[1]), "symbol", json)?;
    report
        .symbols
...
fn analyze_for_cli(path: &Path, command: &str, json: bool) -> NuResult<QueryReport> {
    match analyze_file(path, command) {
        Ok(report) => Ok(report),
        Err(error) => {
            if json {
...
pub fn analyze_file(path: &Path, command: &str) -> NuResult<QueryReport> {
    ...
    let ast = Parser::new(tokens).parse_module_preserving_top_level_lets()?;

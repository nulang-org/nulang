...
mod tests {
    ...
    fn host_abi_requirement_uses_canonical_ids_without_source_spellings() {
        ...
        std::fs::write(&main_path, "perform Storage.write(\"key\", \"value\")\n").unwrap();

        let source = std::fs::read_to_string(&main_path).unwrap();
        let (ast, type_checker, _, _) = checked_module(&source, &main_path, &[], false).unwrap();
        let hir = crate::hir_lower::lower_module(&ast, &type_checker.inferred_decl_types);
        ...
        assert_eq!(
            ...
            vec!["nulang.host-effects/v0alpha1:nulang:storage/string#Write".to_string()]
        );

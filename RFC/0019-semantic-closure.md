...
Current status: source grants are parsed into typed `AuthorityGrant` values and remain structural through AST → HIR → MIR → actor runtime state. Bytecode/native codegen canonicalizes them only at stable artifact compatibility boundaries; persistence and migration keep canonical tokens for format compatibility but validate the complete set before constructing an actor. Bytecode spawn metadata is bound to the exact `Spawn`/`RSpawn` PC, the VM installs the selected manifest on local children, and host authorization checks typed grants directly. Distributed spawn now performs exact-subset delegation on the sending node before creating a placeholder or emitting a packet; authority-bearing remote spawns use the additive NUL0-v1 packet type `SpawnRequestAuth` (type 17), while the frozen type-3 `SpawnRequest` bytes remain unchanged for empty-authority spawns. The receiver validates canonical authority tokens into `AuthorityManifest` before actor creation and installs exactly that typed manifest. This assumes the authenticated/accepted cluster node is the trust boundary for its sender-side delegation checks; cryptographic per-actor delegation proofs are a separate future hardening layer.

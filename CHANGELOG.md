...
- **Cloud reconciliation now has a source-neutral at-least-once event-loop boundary.** Stable event/evaluation IDs drive bounded `process_reconcile_batch` turns; allocation observations can advance only already-owned epochs and cannot resurrect terminal ownership; exact redeliveries coalesce safely; source ACK follows the durable plan/outbox commit so crash-after-commit recovery returns `AlreadyCommitted` instead of duplicating placement work.

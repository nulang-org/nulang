...
- **JSON persistence reuses append file descriptors without weakening durability** (Experimental, `src/runtime/persistence.rs`). Per-actor journal, workflow-event, and event-sourcing handles are opened lazily and cached, eliminating repeated directory traversal and open/close syscalls while retaining `sync_all()` before every append returns. Append records serialize into one reusable byte buffer before any log write, avoiding a fresh JSON `String` while preserving all-or-error serialization. Each log-kind cache is bounded to 64 descriptors so large durable-actor populations cannot cause unbounded FD growth; eviction only closes already-synced handles. Clones start with empty descriptor caches and `clear(actor_id)` drops cached handles before deleting files.

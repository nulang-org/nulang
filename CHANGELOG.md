...
- **The first ABI slice covers the full Core value layout, including `TAG_OBJECT`, and indexes existing versioned runtime contracts.** The top-level manifest also exports the language version, artifact-identity manifest version, Behavior Manifest schema, and host-effect ABI schema from their canonical constants. Authority, WIT world metadata, and typed-program semantic-schema export remain follow-up work under #1046.

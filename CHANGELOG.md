...
- **Incoming workflow commands now advance the same fenced atomic tail before execution.** Command acceptance commits the journal record without advancing the completed-state snapshot, so a crash during execution leaves replayable input above the last completed snapshot. Failed command acceptance requeues the message and prevents its handler from running instead of consuming uncommitted work.
...
- **Externally visible workflow progress is gated on durable commit.** Failed signal commits are not delivered; `TimerFired` commits the event and `step_index` advance in one snapshot, rolls the live mutation back on failure, and re-arms a bounded retry; saga compensation stops at the first completion marker that cannot be durably recorded.

...
- **Selective receive uses a hybrid lookup path**: clauses with fewer than 8 behavior arms scan each staged lane once across guard retries using three transaction-local cursors, while 8+ arm clauses retain lazy per-behavior indexes to avoid O(mailbox depth × arm count) matching.
- The hybrid keeps system → local → normal precedence, FIFO candidate choice, first-arm semantics, and transactional guard retry/reset behavior. Linear-mode arrivals invalidate any older positional index so a later indexed receive rebuilds from authoritative staged buffers.
- Criterion coverage measures mailbox depth, arm count, and repeated guard rejection; focused regressions pin index rebuilds after commit and mode switching, new arrivals, duplicate arms, and cursor rewind on receive reset.

...
### Terminal workflow transition host-wire staging — 2026-09-26
- **Runtime terminal workflow transitions can now be staged into the portable durable host protocol without semantic loss for the supported subset.** Accepted commands, actor state, terminal `StepCompleted` / `StepFailed` records, stable activation identity, and tagged persisted values are encoded deterministically and bound to the protocol digest.
- **Unsupported durability semantics fail closed instead of being omitted.** Timers, signals, saga/parallel/custom records, workflow-start state, snapshot suspension/CRDT/authority metadata, domain events, durable effects, and outbox records are rejected until a lossless mapping exists.
- **Host commit responses are accepted only when owner, epoch, sequence, and digest exactly match the staged request.**

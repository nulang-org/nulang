...
- **Manifest parsing fails closed** on unknown schema versions, malformed content identities, duplicate actors, invalid/incomplete migration chains, and artifact identity tampering. Canonical ordering gives deterministic JSON and a domain-separated BLAKE3 manifest digest.
- **Deployment upgrade preflight rejects structural durable-state hazards**: removing a durable owner, dropping durability, schema downgrades, same-version schema drift, package identity mismatch, or a missing version-by-version migration path. Migration body identity is explicitly `topology-only` in v0alpha1; successful preflight is not represented as proof that runtime migration execution is complete.

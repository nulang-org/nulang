...
- **Compiler-owned `EffectSiteId` analysis** (Experimental, `src/effect_site_identity.rs`). Explicit performed effects are assigned domain-separated BLAKE3 identities from stable semantic owner names, normalized MIR block/statement coordinates, nested-rvalue paths, and qualified effect operations. Source spans, line tables, bytecode PCs, and declaration-vector positions are excluded. `EffectSiteId` is a locator within an owner; durable invocation identity must combine it with the owning `SemanticId` and durable execution context.
### Semantic durable-effect identity — 2026-09-22
- **`DurableEffectId::derive_semantic`** (Experimental, `src/durable_effect.rs`) adds the compiler-owned durable-effect identity path: owner `SemanticId` + `EffectSiteId` + replay-stable durable execution key + occurrence index. It removes physical/runtime actor IDs from new semantic effect identities while preserving the legacy derivation for existing persisted records. Recovery must reuse an already-journaled operation ID across code upgrades rather than re-derive it from newly deployed semantics.

...
### Portable continuation compatibility contract — 2026-09-21
- **Logical suspension sites now have backend-independent identity and live-layout compatibility metadata** (Experimental, `src/continuation_contract.rs`). `ContinuationPointId` excludes bytecode/native PCs, `ContinuationLayoutId` hashes compiler-canonical live semantic state, and the versioned `NUCT` metadata envelope binds a captured continuation to its originating `SemanticId`, activation/message sequence, and effect cursor. Hot-patch compatibility intentionally ignores the old/new whole-code semantic ID but fails closed when the logical point or live layout changes. This is a metadata substrate only; existing VM frame serialization remains unchanged until compiler lowering emits portable live-value layouts.

### Compiler-emitted continuation suspension sidecar — 2026-09-21
- **MIR codegen now emits continuation descriptors alongside bytecode without changing frozen NBC v1** (Experimental, `src/mir_codegen.rs`). Suspension-capable `PerformAsync`, `SignalWait`, and `ReceiveWait` instructions receive exact module-absolute PCs, deterministic logical point IDs, canonical resume/live-layout IDs, and artifact-local register/spill capture plans. Backwards may-liveness determines values that must survive the suspension; compatibility hashes include semantic local keys, canonical types, and reference capabilities but exclude physical register/spill locations. Functions with effect-handler tables are explicitly marked as requiring handler remapping before hot-patch resume can be enabled.

### Typed actor suspension reasons — 2026-09-21
- **Captured actor execution now records why it is suspended** (Experimental, `src/runtime/actor.rs`, `src/runtime/mod.rs`, `src/runtime/llm.rs`). Existing VM suspension kinds map to `SignalWait`, `ReceiveWait`, or `AsyncEffect`, while JIT yields are recorded explicitly as `JitSafepoint`; reserved `DebugPause` and `Fault` variants establish the runtime vocabulary for live debugger/fault suspension without changing current scheduling, restart, or supervision behavior.

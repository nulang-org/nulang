...
- **Package builds now emit `<package>.behavior.json` beside executable artifacts** (`src/behavior_manifest.rs`, `src/main.rs`, `src/package/commands.rs`). Bytecode builds bind the sidecar to exact `.nbc` bytes; `nula build-wasm` binds the same compiler-derived semantic metadata to exact canonical `.wasm` bytes while treating `.cwasm` as a host-specific precompiled derivative. The experimental `nulang.behavior/v0alpha1` sidecar binds package/language metadata to compiler artifact identity and exposes durable actor persistence class, schema version, canonical state-schema semantic identity, and migration topology.

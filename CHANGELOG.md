...
- **The durable allocation outbox now has an idempotent execution dispatcher with stale-Start fencing.** Stop commands run before Starts, failed Stops block replacements for that replica, pending Starts are checked against the current authoritative epoch, and stale or race-lost Starts are compensated with an idempotent Stop before their outbox entry is acknowledged. This makes crash-after-apply recovery safe for a single logical dispatcher per scope; durable multi-dispatcher command claiming remains follow-up work.
- **Allocation command delivery now supports active-active dispatchers through durable leases with generation fencing.** Claims are persisted in the same control-store transaction as other state mutations, expired claims can be reclaimed with a higher generation, stale dispatchers cannot ACK or release the replacement claim, and unresolved Stops prevent Starts for the same replica from being claimed. JSON restart tests pin lease persistence and generation rollover.
- **Node-side allocation execution now has a second durable epoch fence.** `FencedNodeExecutor` persists Starting/Stopping before external workload lifecycle effects, replays exact operations idempotently after failure/restart, blocks newer epochs behind unresolved local ownership, prevents stopped epochs from being resurrected, and lets stale Stops clean an old exact epoch without overwriting a newer running record. The launcher remains an adapter boundary; immutable artifact/manifest launch identity is the next execution contract rather than being inferred from mutable paths or tags.
- **Cloud workload revisions now have an immutable content-addressed launch contract.** `WorkloadRevisionSpec` binds `(deployment_id, revision)` to RFC 0020 executable/behavior-manifest digests, compiler artifact id, target/ABI/backend, package identity, ordered args, and config-key names while excluding mutable artifact locations and secret values. Memory/JSON/PostgreSQL control stores persist the binding idempotently and reject conflicting reuse; `RevisionBoundLifecycle` fails Start closed when the binding is absent or invalid while allowing Stop without artifact-registry availability.

...
- **Service metadata reuses Fabric's existing convergence mechanics.** Local endpoint changes share the process-wide Fabric generation counter and cross-shard metadata channel; complete remote snapshots ignore duplicate/stale generations and bounded export refuses destructive partial snapshots.
- **Service discovery now converges automatically over the existing NUL0 gossip round.** A self-identifying additive `SVC0` tail carries complete generation-tagged service snapshots independently of the existing `FAB0` subscription tail. Sender NodeId is verified before apply, stale generations are not rebroadcast to lagging local shards, and two-node deterministic-network coverage proves Serving → Unhealthy → newer-epoch Serving convergence without a separate discovery daemon.

...
- **The durable allocation outbox now has an idempotent execution dispatcher with stale-Start fencing.** Stop commands run before Starts, failed Stops block replacements for that replica, pending Starts are checked against the current authoritative epoch, and stale or race-lost Starts are compensated with an idempotent Stop before their outbox entry is acknowledged.
- **Active-active dispatch now uses durable replica-group leases.** All pending commands for one `(deployment, replica)` are claimed together with caller-supplied deterministic lease time, preventing competing dispatchers from splitting a Stop/Start transition. Expired claims are reclaimable, claimed ACKs are ownership-checked, and the batch limit yields to replica-group atomicity rather than breaking ordering.

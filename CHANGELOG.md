...
- **A `DurableCacheStore` layer now provides Memory, BufferedJournal, and SyncedJournal acknowledgement modes plus atomic WAL batch records and crash-safe snapshot/WAL rotation.** Journal failures poison the wrapper so a shard cannot continue serving in-memory state that diverged from its durable history; production RESP-server wiring and replica/quorum acknowledgement remain follow-up work.
- **The Mio RESP server now owns a memory or durable command target explicitly, and `nulang-cache` can bootstrap `memory`, `buffered`, or `synced` modes from snapshot + WAL before binding its listener.** Journaled mutation paths cannot access the raw mutable `CacheStore`; startup advances stale/absent WAL sequence bases to the recovered snapshot boundary before accepting writes.
- **Durable cache servers now expose structured durability status and an explicit administrative snapshot/WAL-rotation trigger.** Status reports mode, poison state, and WAL base/tail sequence; snapshotting remains explicit rather than running on the latency-sensitive reactor until asynchronous compaction is available.
- **Cache replication now has a checksummed epoch-fenced wire record and strict ordered replica applier.** Frames reuse the cache placement epoch plus WAL sequence, replicas reject stale/future epochs and sequence gaps before mutation, retransmits are idempotent, and application failures poison the replica; acknowledgement transport and bootstrap transfer remain separate follow-up layers.
- **Primary-side replica acknowledgement tracking now enforces the same placement epoch and monotonic WAL progress.** Acknowledgements from unknown/stale/future replicas or beyond the local WAL tail are rejected, and advancing the placement epoch clears prior-term acknowledgement credit before any new write can count it.
- **Replica bootstrap now reuses the crash-recovery snapshot codec through a bounded manifest/chunk state machine.** Receivers enforce expected placement epoch, maximum snapshot size, contiguous offsets, exact final byte count, and BLAKE3 verification before installing state; the replica then resumes incremental WAL replication at the snapshot sequence.
- **Replica promotion is now fenced by a strictly newer placement epoch and an exact committed WAL sequence.** The acknowledgement tracker computes a replica commit index, while promotion rejects both behind candidates and ahead candidates carrying a potentially uncommitted tail.
- **Live slot migration now has epoch-fenced Redis-compatible ASK/ASKING routing.** Migration begins without changing source ownership, missing source keys redirect with ASK, importing targets require one-shot connection-scoped ASKING authorization, mixed multi-key requests return TRYAGAIN, final ownership cutover requires a second newer placement epoch, and failed transfers can be cancelled under a newer epoch without changing source ownership.

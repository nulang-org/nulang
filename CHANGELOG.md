...
- **JSON persistence reuses append file descriptors without weakening durability** (Experimental, `src/runtime/persistence.rs`). Per-actor journal, workflow-event, and event-sourcing handles are opened lazily and cached, eliminating repeated open/close syscalls while retaining `sync_all()` before every append returns. A store-level reusable byte buffer now serializes both append records and snapshots before any disk write, avoiding fresh JSON `String` allocations while preserving all-or-error serialization; known actor directories are cached to avoid repeated `create_dir_all` path walks on checkpoints. Each log-kind cache is bounded to 64 descriptors so large durable-actor populations cannot cause unbounded FD growth; eviction only closes already-synced handles. Clones start with empty descriptor/directory/scratch caches and `clear(actor_id)` drops cached handles and directory state before deleting files.

...
### In-memory RFC 0008 historical event migration — 2026-09-21
- **Historical event streams can now migrate during recovery without journal rewriting** (Experimental, `src/runtime/event_replay.rs`, `src/runtime/migration.rs`, `src/runtime/mod.rs`). Per-field persistence rows are first collapsed into ordered logical events. Each old event is then advanced through the adjacent migration chain: a named event arm executes its compiler-private transform in an isolated VM and captures zero/one/many `emit` results into an in-memory replay stream; `other => other` advances unmatched events without bytecode; an unmatched event without catch-all fails closed. The final current-schema stream is fed directly into the replay-safe private `apply` projections introduced by #699, preserving one-to-many ordering even when replacements share the same durable sequence. Snapshot migration may now coexist with immutable event history: snapshot transforms are fenced to fields physically present in the old snapshot plus current durable fields, while event-sourced state is reconstructed only from history. Historical event-transform access to `self` remains deliberately denied because RFC 0008 requires old-version state and the runtime cannot yet supply that view faithfully. End-to-end recovery tests pin combined state + event migration and prove the original event rows remain unchanged. Migration topology digests now exclude both state and event artifact-local function indices.

### Executable RFC 0008 event-transform artifacts — 2026-09-21
- **Compiler-private historical event transforms** (Experimental, `src/hir.rs`, `src/hir_lower.rs`, `src/mir_lower.rs`, `src/migration_manifest.rs`, `src/runtime/migration.rs`). Named `migration from V to V+1 { events { | Old(params) => ... } }` arms now survive HIR lowering, receive private function-table slots named `Entity.$migration_event_V_W_Old`, and bind those indices into the canonical migration manifest. Historical parameters remain dynamically typed at this boundary because a current artifact need not retain the old event schema. `| other => other` stays declarative pass-through metadata with no executable binding. Runtime manifest validation checks every bound event-transform index against its compiler-owned canonical function identity and rejects executable catch-all bindings. A regression pins named-arm binding and catch-all non-binding. Runtime execution is provided by the in-memory historical event migration layer described above.

### Executable current-schema event replay — 2026-09-21
- **Isolated apply replay during recovery** (Experimental, `src/runtime/event_replay.rs`, `src/runtime/migration.rs`, `src/runtime/mod.rs`). Recovery can now reconstruct current-schema `event_sourced` state by grouping per-field `EventEntry` rows into one logical event per durable sequence and invoking the compiler-private `Entity.$apply_Event` projection exactly once. The compiler records a fail-closed `replay_safe` proof: existing apply handlers that touch durable/local/CRDT actor state or actor/effect/event surfaces remain source-compatible but keep legacy stored-value recovery. Executable replay additionally validates sibling-row consistency, canonical function identity and arity, executes against an unpublished actor with a runtime state-access fence restricted to declared `event_sourced` fields, and reproduces the runtime's existing post-apply integer `+1` per-emit behavior. Corrupt/inconsistent executable histories abort recovery; unsupported legacy projections fall back to recorded post-apply values. This establishes the current-schema projection stage needed by RFC 0008 event migration without rewriting historical journals.

### Replayable current-schema apply handlers — 2026-09-21
- **Compiler-private event projection functions** (Experimental, `src/hir.rs`, `src/hir_lower.rs`, `src/mir_lower.rs`, `src/bytecode.rs`). Entity `apply | Event(params) => ...` bodies now survive as a second lowered representation in addition to the existing live emit-site inlining. MIR reserves one private function-table slot per current-schema apply handler (canonical name `Entity.$apply_Event`), binds `self` plus ordered event parameters, and records `{ event, param_count, function_index }` in serde-defaulted `ActorMeta.apply_handlers`. These functions are deliberately absent from `func_map`, exports, and the actor behavior table, so source calls and send/ask cannot address them. Live execution semantics are unchanged in this slice; the private artifact is the deterministic replay substrate required before RFC 0008 event migrations can transform historical events and run the current apply projection.
...
- **RFC 0008 validation hardening** (Experimental, `src/typechecker.rs`, `src/effect_checker.rs`). Migration edges must advance exactly one positive schema version, declared chains must be contiguous and end at the entity's current version, migration event parameters/catch-all values are bound during typechecking, and migration state/event bodies are required to have an empty effect row. Runtime snapshot migration and historical event replay are now implemented experimentally; this compiler pass remains the source-level safety gate that prevents malformed or effectful contracts from reaching those executors.

### Virtual-actor schema migration on hydration — 2026-09-21
- **Fenced RFC 0008 preparation for virtual actors** (Experimental, `src/runtime/mod.rs`). The restart migration protocol is now centralized in one `prepare_snapshot_for_module` path and reused by `resolve_or_hydrate_grain`. A persisted old-schema grain executes its state-only migration chain in the isolated migration VM, commits through the same snapshot revision CAS, resolves CAS winners with the same current-schema rule, and only then restores/publishes the grain identity and enqueues the actor. Post-snapshot old-schema journal entries, event/CRDT/workflow state, unsupported persistence CAS, and migration failures still fail closed. Cross-node incoming actor handoff remains a separate follow-up.

### Revision-fenced migration snapshot commits — 2026-09-21
- **Optimistic snapshot CAS for RFC 0008 recovery** (Experimental, `src/runtime/persistence.rs`, `src/runtime/mod.rs`). `PersistenceStore` now exposes a typed `compare_and_swap_snapshot` contract over the durable snapshot revision `(actor_id, sequence, schema_owner, schema_version)`. State migration recovery uses this fence instead of blind snapshot replacement. Memory and libSQL perform revision-checked replacement directly; PostgreSQL uses one conditional `UPDATE` with `IS NOT DISTINCT FROM` for NULL-safe owner equality; the writable RocksDB store uses its exclusive DB writer lock plus revision comparison before WAL-synced replacement. `JsonFileStore` deliberately reports `Unsupported` because temp-file + rename is atomic replacement but not a cross-process CAS. On conflict, recovery never retries a transform against stale input: it reloads the winner and continues only when that winner already committed the current artifact schema; otherwise it fails closed.

### Isolated durable-state migration recovery — 2026-09-21
- **RFC 0008 state-only executor + commit-before-publication** (Experimental, `src/runtime/migration.rs`, `src/runtime/mod.rs`). Registered-module recovery can now execute adjacent state migration transforms in a fresh VM bound directly to an unpublished `Actor`; the staged actor never enters `Runtime::actors`, the scheduler, registries, process groups, or distribution. The migration callback surface permits only heap/state access and records/denies actor messaging, spawning, effects, FFI, async operations, event emission, mailbox/signal operations, distributed operations, and node-identity observation. Successful chains preserve the existing durable sequence, serialize the upgraded `state durable` fields, atomically replace the snapshot, and only then continue normal actor recovery/publication; configured shadow replication occurs only after the local commit. Recovery fails closed for workflows, event-sourced fields/history, CRDT state, event-transform declarations, post-snapshot old-schema message journal entries, missing required durable fields, and unsupported heap-backed durable values. `PersistenceStore::save_snapshot` now documents the required failed-write atomicity contract. Event/CRDT/workflow migration and old-schema cross-node handoff remain follow-ups.

### Compiled migration state transforms — 2026-09-21
- **RFC 0008 private state-transform bytecode** (Experimental, `src/hir.rs`, `src/hir_lower.rs`, `src/mir_lower.rs`, `src/mir_codegen.rs`). State migration bodies now survive AST → HIR → MIR → bytecode as compiler-private function-table entries. They are never inserted into source `func_map` or the actor behavior table, and each manifest transition with a state transform carries an artifact-local function index. Recovery recomputes the canonical internal function name from actor + version transition and rejects out-of-range or mismatched bindings before activation. The migration topology digest deliberately excludes these layout-dependent indices. Runtime invocation/atomic commit and event migration execution remain unimplemented and fail closed.

### Compiled migration topology — 2026-09-21
- **RFC 0008 artifact manifest** (Experimental, `src/migration_manifest.rs`, `src/mir_lower.rs`). The compiler now emits a canonical versioned migration topology into `ActorMeta.migrations` instead of dropping migration metadata at MIR lowering. Adjacent transitions are validated, complete 1→current chains are required, event-arm metadata is canonicalized, and recovery validates the manifest and can deterministically plan an old→current path. This remains fail-closed: executable state/event migration bodies are not yet bound to artifact code, so recovery does not transform old durable state.

### Versioned durable persistence envelope — 2026-09-21
- **Fail-closed durable schema recovery** (Experimental, `src/runtime/persistence.rs`, `src/runtime/spawn.rs`, `src/runtime/mod.rs`). Durable snapshots and event-sourced log entries now carry compiler-owned schema owner + version metadata across JSON, SQLite/libSQL, PostgreSQL, grain dehydration, shadow replication, and actor migration. Legacy records without these fields deserialize as schema version 1. Runtime restart, registered-module recovery, event-only recovery, virtual-actor hydration, and cross-node migration reject incompatible schema versions before persisted state becomes observable. This intentionally does not execute RFC 0008 migrations yet; mismatches remain blocked until the migration executor is implemented.

...
/// Baseline the `PerformDirect` hot path before module-load name caching.
...
fn bench_perform_direct_int_to_float(c: &mut Criterion) {
    let source = "var sum = 0.0; var i = 0; while i < 1000 { sum = sum + perform Int.to_float(i); i = i + 1; }; sum";
    let module = compile(source);
    c.bench_function("vm/perform_direct/int_to_float", |b| {
        b.iter_batched(
            || fresh_vm(&module),
            |mut vm| black_box(vm.run().unwrap()),
            BatchSize::SmallInput,
...
criterion_group!(
    ...
    bench_perform_direct_float_sqrt,
    bench_perform_direct_int_to_float,
    bench_perform_direct_array_length,
    bench_perform_direct_string_length,

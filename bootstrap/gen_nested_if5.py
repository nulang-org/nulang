...
def make_body(depth):
    """Generate a nested if/else body with let bindings."""
    ...
    def gen(level):
        ...
        v = params[level - 1]
        inner = gen(level + 1)
        return f"(if {v} == 1 then (nr * 262144) + p else if {v} == 2 then (let x{level} = {v} + 1 in let y{level} = x{level} * 2 in {inner}) else (nr * 262144) + p)"
    return gen(1)

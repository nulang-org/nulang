...
use nulang::ffi::c_api::{
    ...
    nulang_runtime_new, nulang_value_int, nulang_value_int_new,
};
...
fn cached_public_handle_works_across_c_api_module_operations() {
    ...
    let source =
        CString::new("fn len(s: String) -> Int { perform String.length(s) } len(\"\")").unwrap();
    ...
    assert_ne!(
        first, cached,
        "public compile handles retain fresh identity"
    );
...
#[test]
fn c_api_calls_non_first_function_by_bytecode_offset() {
    let runtime = nulang_runtime_new();
    assert!(!runtime.is_null());

    let source = CString::new(
        "fn first(x: Int) -> Int { x + 1 }\nfn later(x: Int) -> Int { x + 40 }\nfirst(0)",
    )
    .unwrap();
    // SAFETY: runtime and source remain valid for the calls below.
    ...
    let function = CString::new("later").unwrap();
    let arguments = [nulang_value_int_new(2)];
    // SAFETY: runtime, handle, function name, and argument array are valid.
    let result = unsafe {
        nulang_call_function(
            runtime,
            handle,

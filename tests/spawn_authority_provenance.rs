...
"#;
    ...
    // SAFETY: `raw` is the tagged u64 result produced by the AOT-compiled
    // program itself, so it is a valid NaN-tagged `Value` bit pattern.
    assert!(unsafe { nulang::vm::Value::from_bits(raw) }.is_nil());
